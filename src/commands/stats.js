            ...
            .then((rows) => {
                ...
                const statsEmbed = new EmbedBuilder()
                    ...
                    .addFields(
                        ...
                        { name: `${materialIcons.gold}`, value: `${rows[0].gold}`, inline: true },
                        { name: `${materialIcons.silver}`, value: `${rows[0].silver}`, inline: true },
                        { name: `${materialIcons.honorShards}`, value: `${rows[0].honorShards}`, inline: true },
                        { name: `${materialIcons.guardianStone}`, value: `${rows[0].guardianStones}`, inline: true },
                        {
                            name: `${materialIcons.destructionStone}`,
                            value: `${rows[0].destructionStones}`,
                            inline: true,
                        },
                        { name: `${materialIcons.leapstone}`, value: `${rows[0].leapstones}`, inline: true },
                    )
                    .setTimestamp();
